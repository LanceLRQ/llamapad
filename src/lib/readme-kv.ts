 ...
 * 一起卡住）。闭合 `**` 之前的冒号改由下面 `.replace(/:\s*$/, "")` 在捕获组里
 * 事后剥掉：四种规范写法（`**L**` / `**L:**` / `**L**:` / `**L:**:`）与旧正则
 * 逐字符一致；两处空白边界形态有差异——`**L** :`（闭合后空格再冒号）旧正则收、
 * 新正则不收，`**L: **`（冒号后空格再闭合）旧留冒号、新一并剥掉。两者都不是
 * CommonMark 合法的强调闭合，12 份真实语料里也没有，按更一致的新行为处理。
 */
const BOLD_LINE = /^\s*\*\*(.+?)\*\*:?\s*$/;
