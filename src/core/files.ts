 ...
 *
 * K 系后缀写成 `XL|XS|[SML]` 而不是原先的 `[SML]`：unsloth 的 UD 量化用了
 * 两字母后缀（`Q4_K_XL` / `Q6_K_XL`），而 `[SML]` 只吃单字母。**漏匹配在这里
 * 不会退化成"识别失败"，而是静默截断**——`Q4_K_XL` 里的 `Q4_K` 先匹配成功，
 * 其后紧跟的 `_` 不是字母数字，否定前瞻照样放行，于是返回 `Q4_K`。真机
 * unsloth/Qwen3.8-27B-GGUF 里 `UD-Q6_K_XL` 因此被截成 `Q6_K`，与同仓库真的
 * `UD-Q6_K` 撞成同一个标签，两个不同量化在界面上无法区分。
 *
 * 后缀刻意枚举而不写成 `[A-Z]+`：后者会把 `Q4_K_MERGED` 这类非量化后缀
 * 一并吞进标签，而这一层的产出会被当成分组键用。
 */
const QUANT_RE =
  /(^|[^A-Za-z0-9])(IQ\d+_?[A-Z0-9]*|Q\d+_K(?:_(?:XL|XS|[SML]))?|Q\d_\d+|BF16|F16)(?![A-Za-z0-9])/i;
