...
/**
 * Chat / Playground 页（M3 Task 6，设计 §10；真机缺陷修复改三分支；自建 Playground
 * 任务 7 起弃用内嵌 llama.cpp web UI）：server 侧判运行状态三选一渲染——
 * - 运行中且已就绪：面板自建 Playground（`ChatPanel`：对话区 + 只读参数栏），
 *   页头再挂一个「打开 llama UI」外链按钮，作为 llama.cpp 自带 web UI 的补充入口
 ...
 * 无目标端口，引导卡比连接失败页更诚实；此时也无法算合并配置，ChatPanel 拿到的
 * config/ctxSize 均为 null，参数栏不渲染，Playground 照常可用。ready 为 false（容器
 * 起了，llama-server 还没监听）时既不该渲染 Playground（会立刻请求失败）也不该退回
...
export default async function ChatPage() {
  ...
  const merged = row ? mergeConfig(repo.getDefaultConfig(), row.overrides ?? {}) : null;
  ...
  // 不加回来 Playground 底下会空出一条。三个数字在同一个元素上，要改一起改。
  return (
    ...
    <div className="-mx-[34px] -mt-7 -mb-12 flex h-[calc(100%+76px)] min-h-96 flex-col">
      {/* 顶栏：标题 + 运行模型 chip + API 调用示例复制（UX P0 Task 6）+ llama UI 外链 */}
      <PageHeader
        icon={MessageSquare}
        title={t("title")}
