...
### 2.3 判据的适用范围：本地权威、远端预筛

上面这套判据**只适用于已下载到本地的文件**——它要读 GGUF 元数据。两条链路因此分开：

| 链路 | 面对的对象 | 判据 | 效力 |
|---|---|---|---|
| 建配置候选（`batch-create.ts`） | 已下载完整的本地文件 | GGUF 元数据 | 权威判定，直接排除 sidecar |
| 下载向导 | 远端仓库清单，尚无本地文件 | `MTP/` 目录段与 `mtp-` 前缀 | **仅用于建议勾选**，不做硬判定 |

远端其实也能读元数据（HF 支持 Range 请求，取前 2MB 即可，实测与本地逐字段一致，见调研补记二），但那要新开一条带缓存与超时的远端探测链路，不在本期（见 §9）。
...
`gguf_meta` 表（迁移 v6）是**显式列**的表（`ggufMeta.ts:99` 的 SELECT 列清单），加字段绕不开 schema，必须写迁移 v19。

手法照 v12 给该表加 `chat_template` 那次：**DROP + CREATE，不用 `ALTER TABLE ADD COLUMN`**。v12 的注释已写明理由——存量行的新列会被补成 NULL，而 NULL 在这里天生歧义：分不清是「这个 GGUF 确实不含 MTP」还是「旧版本压根没采这一列」。前者该判 `none`（开关置灰），后者该重新解析，混在一起会让老部署升级后已缓存的内嵌型模型永远开不了 MTP。`gguf_meta` 是纯缓存（v8 `file_meta` 注释写明的语义），DROP 重建零风险。
