...
在 `src/lib/model-form.test.ts` 末尾追加（复用该文件既有的 `BASE` / `EMPTY` 工厂，`:21-30`）：
...
在 `src/lib/preset-draft.test.ts` 末尾追加（复用该文件既有的 `emptyDraft` 工厂，`:6-15`）：
...
> `emptyDraft` 是一份完整的 `DraftState` 字面量，**给 `DraftState` 加字段会让它直接编译失败**。
> 在它的 `gpuLayers: ""` 那一行旁补上 `splitMode: "", tensorSplit: "", mainGpu: "",`。
> 这是全库唯一一处完整 `DraftState` 字面量（已全量核查），其余构造点一律走 `initDrafts`。
...
预期：PASS。唯一需要同步的完整 `DraftState` 字面量是 `preset-draft.test.ts` 的 `emptyDraft`（步骤 1 已说明）；`wizard.tsx` / `edit-form.tsx` / `duplicate-form.tsx` 都走 `initDrafts` 构造，不用改。
...
其中 `splitModeLabel` 是同组件内的一个小辅助（放在 `hintFor` 旁边），用显式映射避免拼接 i18n 键名：
...
> `row` 在下拉里排在 `tensor` 之后，是因为它已被上游弃用——不给它默认视线位置。
