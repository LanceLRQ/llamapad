...
| 批 | 内容 | 依赖 | 交付后可用性 |
|---|---|---|---|
| ① 底层 | 匹配判定、v17 迁移、inode 去重、挂载表发现、file_meta union | — | 无 UI 变化，但存储空间占用统计修正立即生效 |
 ...
 * 组级默认动作按「代价最低」在**交集里**挑，而不是比较各文件的默认值是否一致：
 * move（单份、不额外存储空间占用）> link（单份、原地不动）> copy（占额外盘但离线可行）
 * > download（永远可行的兜底）。download 恒在交集里，所以总能选出结果。
 *
 * 曾经写成「各文件默认动作一致就取它、否则降到 download」，那是错的：一组里
 * 一片在别的档案（只能 link）、另一片游离在 models 下（可 move 可 link），两片的
 * 默认值不一致，但整组统一走 link 完全可行——降到 download 会让用户白下几十 GB，
 * 正好背离本里程碑的目标。判据是「交集里还剩什么」，不是「大家本来想干什么」。
 *
...
export function mergeGroupMatch(
  ...
  // 只有「确实用得上本地副本」的文件参与组级动作推导。没找到候选的、以及找到了
  // 但远端没有 oid 无从确认的，它们的 actions 只剩 download；把它们算进交集会把
  // 整组拖成「只能下载」——3 片里缺 1 片就要把已经在盘上的 2 片重下几十 GB，
  // 正好背离本里程碑。设计 §4.4 写得很明白：「组的默认动作取组内**已有文件**的
  // 位置推出；组内没有对应本地文件的分片，该文件走下载」，两者同批执行。
  ...
  // 组级 restriction 从**全部** files 里取第一个非 none 的（不是从 basis）：
  // 被排除出 basis 的文件恰恰是最需要解释的那些（「这片没有校验值，只能下载」），
  // 从 basis 取会把这条信息丢掉
...
git commit -m "feat(local-acquire): 扫描带 inode，档案存储空间占用按 inode 去重"
```
 ...
 * 注释的既有结论，此处沿用同一手法。
 */
// 模块级变量存在 globalThis 上而不是普通模块变量：Next dev 的热重载会重新求值
// 模块，普通变量会被清零。写法照 server 目录既有的 6 个单例（locators.ts:32、
// panelConfig.ts:27、proxyAgentCache.ts:32 …）——类型化交叉 + 驼峰键名，
// 不要写成 `(globalThis as Record<string, unknown>)[KEY]`：那样键名打错字
// tsc 抓不到，而这个目录里所有同类单例都靠类型化写法守着
 ...
 *  **这层要单测**：`discoverHostModelsRoot` 在 `selfMounts.test.ts:52-89` 有 5 个用例
 *  （非容器环境 / 自身查不到 / 无命中 / docker 抛错 / 命中成功），借 `createMockDockerAdapter`
 *  实现。本函数是启动路径上无条件执行的唯一新增异步函数，同样照那 5 条路径覆盖，
 *  别只测纯函数那一层 */
...
// 这条要**真的在根外造一个文件**再断言它不在结果里。写成「跑一遍 listFileMeta
// 然后断言没有以 / 开头的 path」是假测试：什么都不造的话该断言在改动前后都恒真，
// 挂着强断言的外壳却对「根外文件确实被拒」零保护
...
// 游离的分片组按**物理文件**逐个登记（不像已配置的分片组那样合并成一行 glob）——
// 这是新口径里唯一区别于既有「逻辑条目」惯例的地方，刻意如此：后续批次的 L2 校验
// 要拿每个物理文件各自的 sha256 去比远端 oid。用测试钉住，免得后来者当 bug 修掉
it("游离分片组按物理文件逐行登记，不合并成 glob", async () => {
  ...
  expect(paths.sort()).toEqual([
    "loose/big-00001-of-00002.gguf",
    "loose/big-00002-of-00002.gguf",
  ]);
});
```
...
async function enqueueLocal(args: EnqueueLocalArgs): Promise<{ taskIds: number[]; batchId: string }> {
  ...
  // 但跳过磁盘空间预检的网络部分：link 不占用存储空间、move 同盘不占用存储空间，
  ...
  for (const g of scanTree(modelsRoot)) {
    for (const f of g.files) {
      candidates.push({
        ...
        inModelsRoot: true,
        hostPath: toHost(path.join(modelsRoot, f.rel)),
      });
    }
  }
    ...
    for (const g of scanTree(panelDir)) {
      for (const f of g.files) {
        candidates.push({
          ...
          inRepoDir: null,
          inModelsRoot: false,
          // 这一支的 panel 路径由 toPanel(hostDir) 换算而来，往回换必然成立
          hostPath: toHost(path.join(panelDir, f.rel)),
        });
      }
    }
  }
...
运行：`pnpm exec vitest run src/lib/acquire-plan.test.ts`
预期：PASS，10 个用例（含一条 `groupKey` 用例：两组 quant 都是 null 时 key 不相撞）
...
"acquireHintLink": "创建硬链接，两处共用同一份数据，不额外存储空间占用",
...
"acquireMixedDownload": "其中 {count} 个文件本地没有，将照常下载",
"scanAction": "扫描",
"scanning": "扫描中…",
...
async function onAcquireSubmit(): Promise<void> {
  ...
  const items = acquireRows.flatMap((row) =>
    row.files.map((f) => {
      ...
      // 这里做：服务端见到 action≠download 却不带源路径会直接 400 SOURCE_REQUIRED，
      // 那是防篡改的正确防御，不该为混合组放宽
...
> `f.candidate.hostPath` 是 `LocalCandidate` 的宿主机视角字段，任务 1 已定义、任务 12 的
> scan API 用 `toHost(absPath)` 填值——前端展示与回传都用宿主机路径（项目约定），panel
> 视角只在服务端内部流转。本任务直接取用即可，不需要再改类型。
