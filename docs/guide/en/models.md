...
A model with a "config currently running" also carries extra restrictions: deleting its config is blocked (409) while it's running, and so is changing its namespace or moving its physical files (also 409 — other running models that share the same file are blocked the same way). The shared premise: something a container is actively using can't be changed while it's in use.
...
Config is split into four sections: Basic info (display name / namespace / GGUF and mmproj paths), Docker (container name / port / image / GPU), Performance (context size, GPU layers, K/V cache type, etc.), and Sampling (temperature, Top-p, Top-k, etc.). A namespace is only a config grouping label and is unrelated to which disk directory a file actually sits in — changing a namespace never moves any files; a GGUF path supports shard globs (e.g. `main/Qwen-*-00001-of-00002.gguf`), and can reference the same physical file from multiple namespaces.
...
- **The template recognizes the variable but validates its range**: if you pass a value outside what the template allows, the container still starts up and passes its health check normally — **the error only surfaces as an HTTP 500 when you actually send an inference request carrying this parameter** (the error message comes from the template's own validation and looks like a jinja error). The panel validates this before saving a config and before every start, using the value range the template actually allows to block obviously out-of-range values; but if the panel can't determine whether a model supports this feature (no embedded template, or it's supported but the specific levels couldn't be parsed), it won't force a block — it will only show a hint
