...
Absolute paths in the panel's config (things like the model library location and mount mappings) are written using the host's directory structure, not the path as seen from inside the panel's own container, because mounting a directory into a model container at start only understands host paths. A model's own GGUF path is a separate matter: it's a path relative to the model library root. See [Deployment & Operations](./deployment.md).
