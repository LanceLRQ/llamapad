...
- **Direct URL**: enter an http/https link directly; this doesn't go through the repo-profile layer, and the filename on disk defaults to the last segment of the link (you can also specify it manually). The target directory can't be inside any registered repo profile directory: those are managed exclusively by the profile, and mixing freely downloaded files in would distort its "has this quant been downloaded" bookkeeping, so picking one is refused outright
