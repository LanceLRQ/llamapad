...
面板自建 Playground **不会主动发送任何采样参数**：请求体只有 `model`、`messages` 与 `stream` 三个字段，其中 `model` 只用来指定发往哪个模型，不是采样参数。llama.cpp 自带 web UI 的采样参数配置存在 `:18080`（llama-server 自己的端口）那个 origin 的 `localStorage` 里，面板跨源读不到也改不到；面板自己不发采样参数，就从根上绕开了两边配置对不上的问题，模型模板与配置里已经设置好的默认值会照常生效。
...
**发往同一个模型的并发请求共用一个模型实例。** 能同时处理多少请求取决于 llama-server 自身的 slot 数量，面板不做排队也不做限流。
